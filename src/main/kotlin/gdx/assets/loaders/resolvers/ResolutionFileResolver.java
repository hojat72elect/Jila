...
public class ResolutionFileResolver implements FileHandleResolver {
    ...
    protected final gdx.assets.loaders.resolvers.ResolutionFileResolver.Resolution[] descriptors;
    /**
     ...
     */
    public ResolutionFileResolver(FileHandleResolver baseResolver, gdx.assets.loaders.resolvers.ResolutionFileResolver.Resolution... descriptors) {
    ...
    static public gdx.assets.loaders.resolvers.ResolutionFileResolver.Resolution choose(gdx.assets.loaders.resolvers.ResolutionFileResolver.Resolution... descriptors) {
        int w = Gdx.graphics.getBackBufferWidth(), h = Gdx.graphics.getBackBufferHeight();

        // Prefer the shortest side.
        gdx.assets.loaders.resolvers.ResolutionFileResolver.Resolution best = descriptors[0];
        if (w < h) {
            for (int i = 0, n = descriptors.length; i < n; i++) {
                gdx.assets.loaders.resolvers.ResolutionFileResolver.Resolution other = descriptors[i];
    ...
    public FileHandle resolve(String fileName) {
        gdx.assets.loaders.resolvers.ResolutionFileResolver.Resolution bestResolution = choose(descriptors);
