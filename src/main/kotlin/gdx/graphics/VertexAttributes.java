...
public final class VertexAttributes implements Iterable<VertexAttribute>, Comparable<VertexAttributes> {
    ...
    private gdx.graphics.VertexAttributes.ReadonlyIterable<VertexAttribute> iterable;
    ...
    public java.util.Iterator<VertexAttribute> iterator() {
        if (iterable == null)
            iterable = new gdx.graphics.VertexAttributes.ReadonlyIterable<VertexAttribute>(attributes);
    ...
    static private class ReadonlyIterator<T> implements java.util.Iterator<T>, Iterable<T> {
        ...
        @Override
        public T next() {
            if (index >= array.length) throw new java.util.NoSuchElementException(String.valueOf(index));
            if (!valid) throw new GdxRuntimeException("#iterator() cannot be used nested.");
            return array[index++];
        }
    ...
    static private class ReadonlyIterable<T> implements Iterable<T> {
        private final T[] array;
        private gdx.graphics.VertexAttributes.ReadonlyIterator iterator1, iterator2;
        ...
        @Override
        public java.util.Iterator<T> iterator() {
            if (Collections.allocateIterators) return new gdx.graphics.VertexAttributes.ReadonlyIterator(array);
            if (iterator1 == null) {
                iterator1 = new gdx.graphics.VertexAttributes.ReadonlyIterator(array);
                iterator2 = new gdx.graphics.VertexAttributes.ReadonlyIterator(array);
            }
            if (!iterator1.valid) {
                iterator1.index = 0;
