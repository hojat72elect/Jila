...
import gdx.Files;
import gdx.Files.FileType;
import gdx.Gdx;
import gdx.utils.GdxRuntimeException;
import gdx.utils.StreamUtils;
...
public class FileHandle {
    ...
    protected FileHandle(java.io.File file, FileType type) {
        this.file = file;
        this.type = type;
    }
    ...
    static public FileHandle tempDirectory(String prefix) {
        try {
            java.io.File file = java.io.File.createTempFile(prefix, null);
            if (!file.delete()) throw new java.io.IOException("Unable to delete temp file: " + file);
            if (!file.mkdir()) throw new java.io.IOException("Unable to create temp directory: " + file);
    ...
    static private void emptyDirectory(java.io.File file, boolean preserveTree) {
        if (file.exists()) {
    ...
    static private boolean deleteDirectory(java.io.File file) {
    ...
    public java.io.File file() {
        if (type == FileType.External) return new java.io.File(Gdx.files.getExternalStoragePath(), file.getPath());
        return file;
    }
    ...
    public java.io.InputStream read() {
        if (type == FileType.Classpath || (type == FileType.Internal && !file().exists())
            ...
            java.io.InputStream input = FileHandle.class.getResourceAsStream("/" + file.getPath().replace('\\', '/'));
    ...
    public java.io.BufferedInputStream read(int bufferSize) {
        return new java.io.BufferedInputStream(read(), bufferSize);
    }
     ...
     * @throws GdxRuntimeException if the file handle represents a directory, doesn't exist, or could not be read.
     */
    public java.io.Reader reader(String charset) {
        java.io.InputStream stream = read();
        try {
    ...
    public java.io.BufferedReader reader(int bufferSize) {
        return new java.io.BufferedReader(new java.io.InputStreamReader(read()), bufferSize);
    }
     ...
     * @throws GdxRuntimeException if the file handle represents a directory, doesn't exist, or could not be read.
     */
    public java.io.BufferedReader reader(int bufferSize, String charset) {
        try {
            return new java.io.BufferedReader(new java.io.InputStreamReader(read(), charset), bufferSize);
        } catch (java.io.UnsupportedEncodingException ex) {
            throw new GdxRuntimeException("Error reading file: " + this, ex);
        }
    }
    ...
    public String readString(String charset) {
        ...
        try {
            if (charset == null)
                reader = new java.io.InputStreamReader(read());
            else
                reader = new java.io.InputStreamReader(read(), charset);
    ...
    public byte[] readBytes() {
        java.io.InputStream input = read();
        try {
    ...
    public java.nio.ByteBuffer map(java.nio.channels.FileChannel.MapMode mode) {
        ...
        try {
            java.io.File f = file();
            raf = new java.io.RandomAccessFile(f, mode == java.nio.channels.FileChannel.MapMode.READ_ONLY ? "r" : "rw");
            java.nio.channels.FileChannel fileChannel = raf.getChannel();
            java.nio.ByteBuffer map = fileChannel.map(mode, 0, f.length());
     ...
     *                             {@link FileType#Internal} file, or if it could not be written.
     */
    public java.io.OutputStream write(boolean append, int bufferSize) {
        return new java.io.BufferedOutputStream(write(append), bufferSize);
    }
     ...
     *                             {@link FileType#Internal} file, or if it could not be written.
     */
    public void write(java.io.InputStream input, boolean append) {
    ...
    public java.io.Writer writer(boolean append, String charset) {
        ...
        parent().mkdirs();
        try {
            java.io.FileOutputStream output = new java.io.FileOutputStream(file(), append);
            if (charset == null)
    ...
    public void writeBytes(byte[] bytes, boolean append) {
        java.io.OutputStream output = write(append);
        try {
            output.write(bytes);
        } catch (java.io.IOException ex) {
            throw new GdxRuntimeException("Error writing file: " + file + " (" + type + ")", ex);
    ...
    public FileHandle child(String name) {
        if (file.getPath().length() == 0) return new FileHandle(new java.io.File(name), type);
        return new FileHandle(new java.io.File(file, name), type);
    }
    ...
    public FileHandle sibling(String name) {
        ...
        return new FileHandle(new java.io.File(file.getParent(), name), type);
    }
