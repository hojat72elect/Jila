...
import gdx.math.Plane.PlaneSide;
import gdx.math.collision.BoundingBox;
import gdx.math.collision.OrientedBoundingBox;
import gdx.math.collision.Ray;
import gdx.utils.Array;
import gdx.utils.FloatArray;
...
public final class Intersector {
    ...
    public static boolean intersectPolygons(Polygon p1, Polygon p2, Polygon overlap) {
        ...
        Vector2 ip = gdx.math.Intersector.ip, ep1 = gdx.math.Intersector.ep1, ep2 = gdx.math.Intersector.ep2, s = gdx.math.Intersector.s, e = gdx.math.Intersector.e;
        FloatArray floatArray = gdx.math.Intersector.floatArray, floatArray2 = gdx.math.Intersector.floatArray2;
        floatArray.clear();
        floatArray2.clear();
        ...
        for (int i = 0, last = vertices2.length - 2; i <= last; i += 2) {
            ...
            for (int j = 0; j < floatArray2.size; j += 2) {
                ...
                boolean side = gdx.math.Intersector.pointLineSide(ep2, ep1, s) > 0;
                if (gdx.math.Intersector.pointLineSide(ep2, ep1, e) > 0) {
                    if (!side) {
                        gdx.math.Intersector.intersectLines(s, e, ep1, ep2, ip);
    ...
    static public boolean intersectPolygons(FloatArray polygon1, FloatArray polygon2) {
        if (gdx.math.Intersector.isPointInPolygon(polygon1.items, 0, polygon1.size, polygon2.items[0], polygon2.items[1]))
            return true;
        if (gdx.math.Intersector.isPointInPolygon(polygon2.items, 0, polygon2.size, polygon1.items[0], polygon1.items[1]))
            return true;
     ...
     * @return Whether the line segment and the circle intersect
     */
    public static boolean intersectSegmentCircle(Vector2 start, Vector2 end, Circle circle, gdx.math.Intersector.MinimumTranslationVector mtv) {
    ...
    public static boolean intersectRayTriangle(Ray ray, Vector3 t1, Vector3 t2, Vector3 t3, Vector3 intersection) {
        ...
        if (MathUtils.isZero(det)) {
            p.set(t1, t2, t3);
            if (p.testPoint(ray.origin) == PlaneSide.OnPlane && gdx.math.Intersector.isPointInTriangle(ray.origin, t1, t2, t3)) {
                if (intersection != null) intersection.set(ray.origin);
                return true;
            }
            return false;
     ...
     * @return Whether polygons overlap.
     */
    public static boolean overlapConvexPolygons(Polygon p1, Polygon p2, gdx.math.Intersector.MinimumTranslationVector mtv) {
     ...
     */
    public static boolean overlapConvexPolygons(float[] verts1, float[] verts2, gdx.math.Intersector.MinimumTranslationVector mtv) {
    ...
    public static boolean overlapConvexPolygons(float[] verts1, int offset1, int count1, float[] verts2, int offset2, int count2,
                                                gdx.math.Intersector.MinimumTranslationVector mtv) {
        boolean overlaps;
        if (mtv != null) {
    ...
    private static boolean overlapsOnAxisOfShape(float[] verts1, int offset1, int count1, float[] verts2, int offset2, int count2,
                                                 gdx.math.Intersector.MinimumTranslationVector mtv, boolean shapesShifted) {
     ...
     */
    public static void splitTriangle(float[] triangle, Plane plane, gdx.math.Intersector.SplitTriangle split) {
    ...
    private static void splitEdge(float[] vertices, int s, int e, int stride, Plane plane, float[] split, int offset) {
        float t = gdx.math.Intersector.intersectLinePlane(vertices[s], vertices[s + 1], vertices[s + 2], vertices[e], vertices[e + 1],
    ...
    public static class SplitTriangle {
        ...
        public String toString() {
            return "SplitTriangle [front=" + java.util.Arrays.toString(front) + ", back=" + java.util.Arrays.toString(back) + ", numFront=" + numFront
