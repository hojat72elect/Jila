...
class TimSort<T> {
    ...
    static <T> void sort(T[] a, int lo, int hi, Comparator<? super T> c) {
        ...
        gdx.utils.TimSort<T> ts = new gdx.utils.TimSort<T>(a, c);
     ...
     */
    private static <T> int countRunAndMakeAscending(T[] a, int lo, int hi, java.util.Comparator<? super T> c) {
        if (DEBUG) assert lo < hi;
        int runHi = lo + 1;
     ...
     */
    private static <T> int gallopLeft(T key, T[] a, int base, int len, int hint, java.util.Comparator<? super T> c) {
     ...
     */
    private static <T> int gallopRight(T key, T[] a, int base, int len, int hint, java.util.Comparator<? super T> c) {
        if (DEBUG) assert len > 0 && hint >= 0 && hint < len;
    ...
    public void doSort(T[] a, java.util.Comparator<T> c, int lo, int hi) {
        stackSize = 0;
        rangeCheck(a.length, lo, hi);
        int nRemaining = hi - lo;
        if (nRemaining < 2) return; // Arrays of size 0 and 1 are always sorted
