...
public class Queue<T> implements Iterable<T> {
    ...
    public java.util.Iterator<T> iterator() {
        if (Collections.allocateIterators) return new gdx.utils.Queue.QueueIterator(this, true);
        if (iterable == null) iterable = new gdx.utils.Queue.QueueIterable(this);
    ...
    static public class QueueIterator<T> implements java.util.Iterator<T>, Iterable<T> {
...
// System.out.println(iterable.lastAcquire);
        ...
        public T next() {
            if (index >= queue.size) throw new java.util.NoSuchElementException(String.valueOf(index));
            if (!valid) {
// System.out.println(iterable.lastAcquire);
                throw new GdxRuntimeException("#iterator() cannot be used nested.");
            }
    ...
    static public class QueueIterable<T> implements Iterable<T> {
        private final Queue<T> queue;
        private final boolean allowRemove;
        private Queue.QueueIterator iterator1, iterator2;
         ...
         */
        public java.util.Iterator<T> iterator() {
            if (Collections.allocateIterators) return new Queue.QueueIterator(queue, allowRemove);

            if (iterator1 == null) {
                iterator1 = new Queue.QueueIterator(queue, allowRemove);
                iterator2 = new Queue.QueueIterator(queue, allowRemove);
            }
            if (!iterator1.valid) {
                iterator1.index = 0;
