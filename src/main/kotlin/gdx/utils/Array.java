...
import gdx.utils.reflect.ArrayReflection;
...
public class Array<T> implements Iterable<T> {
     ...
     */
    public Array(gdx.utils.Array<? extends T> array) {
     ...
     * @see #Array(Class)
     */
    static public <T> gdx.utils.Array<T> of(Class<T> arrayType) {
        return new gdx.utils.Array(arrayType);
    }

    /**
     * @see #Array(boolean, int, Class)
     */
    static public <T> gdx.utils.Array<T> of(boolean ordered, int capacity, Class<T> arrayType) {
        return new gdx.utils.Array(ordered, capacity, arrayType);
    }

    /**
     * @see #Array(Object[])
     */
    static public <T> gdx.utils.Array<T> with(T... array) {
        return new gdx.utils.Array(array);
    }
    ...
    public void addAll(gdx.utils.Array<? extends T> array) {
        addAll(array.items, 0, array.size);
    }

    public void addAll(gdx.utils.Array<? extends T> array, int start, int count) {
     ...
     * @param identity If true, == comparison will be used. If false, .equals() comparison will be used.
     */
    public boolean containsAll(gdx.utils.Array<? extends T> values, boolean identity) {
     ...
     * @param identity If true, == comparison will be used. If false, .equals() comparison will be used.
     */
    public boolean containsAny(gdx.utils.Array<? extends T> values, boolean identity) {
        T[] items = values.items;
        for (int i = 0, n = values.size; i < n; i++)
     ...
     */
    public boolean removeAll(gdx.utils.Array<? extends T> array, boolean identity) {
        int size = this.size;
        int startSize = size;
        T[] items = this.items;
        if (identity) {
    ...
    public void clear() {
        java.util.Arrays.fill(items, 0, size, null);
        size = 0;
    }
     ...
     * array must have a size greater than 0, or a {@link gdx.utils.GdxRuntimeException} will be thrown.
     *
     ...
     */
    public gdx.utils.Array.ArrayIterator<T> iterator() {
        if (Collections.allocateIterators) return new gdx.utils.Array.ArrayIterator(this, true);
        if (iterable == null) iterable = new gdx.utils.Array.ArrayIterable(this);
    ...
    public boolean equals(Object object) {
        ...
        if (!(object instanceof gdx.utils.Array)) return false;
        gdx.utils.Array array = (gdx.utils.Array) object;
    ...
    static public class ArrayIterator<T> implements java.util.Iterator<T>, Iterable<T> {
        private final gdx.utils.Array<T> array;
...
// ArrayIterable<T> iterable;
        ...
        public ArrayIterator(gdx.utils.Array<T> array) {
            this(array, true);
        }

        public ArrayIterator(gdx.utils.Array<T> array, boolean allowRemove) {
            this.array = array;
...
// System.out.println(iterable.lastAcquire);
        ...
        public T next() {
            if (index >= array.size) throw new java.util.NoSuchElementException(String.valueOf(index));
            if (!valid) {
// System.out.println(iterable.lastAcquire);
                throw new GdxRuntimeException("#iterator() cannot be used nested.");
            }
    ...
    static public class ArrayIterable<T> implements Iterable<T> {
        private final gdx.utils.Array<T> array;
        private final boolean allowRemove;
        private gdx.utils.Array.ArrayIterator iterator1, iterator2;

// java.io.StringWriter lastAcquire = new java.io.StringWriter();

        public ArrayIterable(gdx.utils.Array<T> array) {
            this(array, true);
        }

        public ArrayIterable(gdx.utils.Array<T> array, boolean allowRemove) {
            this.array = array;
            this.allowRemove = allowRemove;
        }
         ...
         */
        public gdx.utils.Array.ArrayIterator<T> iterator() {
            if (Collections.allocateIterators) return new gdx.utils.Array.ArrayIterator(array, allowRemove);
// lastAcquire.getBuffer().setLength(0);
// new Throwable().printStackTrace(new java.io.PrintWriter(lastAcquire));
            if (iterator1 == null) {
                iterator1 = new gdx.utils.Array.ArrayIterator(array, allowRemove);
                iterator2 = new gdx.utils.Array.ArrayIterator(array, allowRemove);
